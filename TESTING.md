...
> 最近一轮：2026-08-29 第三批（IMPROVEMENT_PLAN 第三批 P2 项 + 回归，全部通过）
> 前两轮：第二批（目录级 API/软删除/分页/分块上传/撤销/队列）、首批（暗色模式/键盘导航/mkdir/缩略图）
...
- IMPROVEMENT_PLAN 剩余项（A9 i18n、B7 面包屑同级下拉、B8 多选拖拽、C4 卡片微动效、C8 图标扩展）待后续批次。
