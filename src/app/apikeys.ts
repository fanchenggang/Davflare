...
export function formatApiUsage(origin: string, apiKey = "<apiKey>") {
  ...
  return [
    ...
    "  文件始终带 size（字节）、uploaded（ISO 时间；缺省时用纪元）和 etag（R2 的 httpEtag 或 etag），",
    "  另有 updated 作为 uploaded 的别名。",
    "  分隔前缀文件夹：isDir 为 true，size 为 0，uploaded 为 JSON null（未知，不伪造 mtime）。",
    "密钥无效或已过期：401",
