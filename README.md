...
- **HamHome migration (read-only):** the library page imports from a [HamHome](https://github.com/bingoYB/ham_home) sync directory on the same instance — reads `/HamHomeSync/meta.json` + `categories.json` and merges by URL. Descriptions become notes, tags and category folders survive, deleted rows are skipped. Boundary: our own writes stay in our format; HamHome snapshots/workspaces/tab rules (stored in its IndexedDB or app-internal JSON) are not migrated.
